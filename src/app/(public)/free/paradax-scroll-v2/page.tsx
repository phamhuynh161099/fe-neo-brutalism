...
const SnapScrollingSections = () => {
  ...
  const sections = [
    ...
    },
    {
      ...
      content: 'We blend creativity with technology to create extraordinary solutions',
      height: '100vh',
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      for (let i = 0; i < sections.length; i++) {
        const sectionHeight = sections[i].height === '100vh' 
          ? containerHeight 
          : containerHeight * 0.5;
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden bg-black">
                  ...
                  {section.id === 'about' && (
                    <div className="grid md:grid-cols-3 gap-6 max-w-6xl mx-auto">
                      {['Innovation', 'Quality', 'Excellence'].map((item, i) => (
                        <GlassCard key={item} className="p-8 hover:scale-105 transition-transform duration-500 text-center" 
                          style={{ animationDelay: `${i * 0.2}s` }}>
                          <div className="text-6xl mb-4">✨</div>
                          <h3 className="text-xl font-bold mb-3 text-white">{item}</h3>
                  ...
                  {section.id === 'services' && (
                    <div className="max-w-4xl mx-auto">
                      <div className="flex flex-wrap justify-center gap-4">
                        {['Web Design', 'Mobile Apps', 'AI Solutions', 'Cloud Services', 'Consulting'].map((service, i) => (
                          <GlassCard key={service} className="px-6 py-3 hover:scale-110 transition-all duration-300">
                  ...
                  {section.id === 'portfolio' && (
                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">
                      {[1, 2, 3, 4, 5, 6].map((item) => (
                        <GlassCard key={item} className="aspect-square p-6 hover:scale-105 transition-transform duration-500 group">
                          <div className="h-full flex items-center justify-center">
                            <div className="text-center">
                              <div className="text-4xl mb-4 group-hover:scale-110 transition-transform">🎨</div>
                              <h3 className="text-lg font-bold text-white">Project {item}</h3>
                            </div>
                          </div>
                        </GlassCard>
                      ))}
                  ...
                  {section.id === 'footer' && (
                    <div className="space-y-8 max-w-2xl mx-auto">
                      <GlassCard className="p-6">
                        <div className="flex justify-center gap-8 text-lg">
                          {['Privacy', 'Terms', 'Support'].map((item) => (
                            <a key={item} href="#" className="hover:text-blue-300 transition-colors duration-300 font-light">
                              {item}
                            </a>
                          ))}
                        </div>
                      ...
                      <div className="space-y-3 text-white/60 text-center">
                        <p className="text-sm font-light">© 2024 Premium Design Studio</p>
                        <p className="text-xs opacity-50">Crafted with ❤️ using Apple Glass Design</p>
                      </div>
                    </div>
                  )}
                </div>
      ...
      <div className="fixed bottom-6 left-1/2 transform -translate-x-1/2 z-40">
        <GlassCard className={`px-6 py-3 transition-all duration-300 ${isScrolling ? 'opacity-100' : 'opacity-70'}`}>
